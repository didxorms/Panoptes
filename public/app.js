...
function render(data) {
  ...
  const routeCards = routes
    ...
    .join('');
  const workerNames = p.engine === 'openprover' ? ['OpenProver'] : ['Atlas', 'Iris', 'Themis'];
  const workers = workerNames
    .map((name, i) => {
      const task =
        tasks.find((t) => t.worker === name && t.status === 'leased') ||
        tasks.filter((t) => t.worker === name).at(-1);
      return `<div class="worker"><div class="worker-icon">${p.engine === 'openprover' ? 'O' : ['A', 'I', 'T'][i]}</div><strong>${name}</strong><small>${p.engine === 'openprover' ? 'Planner · 3 parallel workers' : task ? escape(label(task.kind)) : 'Research agent'}</small>${task ? badge(task.status) : badge('queued')}<p>${task?.checkpoint ? escape(task.checkpoint.slice(0, 110)) : p.engine === 'openprover' ? 'Plans, tests Lean candidates, and keeps a durable research notebook.' : 'Ready to explore, prove, or build on another agent’s result.'}</p></div>`;
    })
    .join('');
  const records = artifacts
    ...
    .join('');
  $('#content').innerHTML =
    `<div class="project-heading"><div><h2>${escape(p.title)}</h2><p>${escape(p.description)}</p></div><div class="project-actions">${p.engine === 'openprover' ? '<span class="badge verified">OpenProver</span>' : ''}${badge(p.status)}${['draft', 'paused'].includes(p.status) ? '<button class="button small" data-action="start">Start research</button>' : ''}${p.status === 'running' ? '<button class="button small" data-action="pause">Pause</button>' : ''}</div></div>
