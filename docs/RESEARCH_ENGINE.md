...
상태: 장기 설계안. v0.1.0은 OpenProver 기반 계획자·병렬 작업자와 Panoptes의 다중 기여자 예산·격리 검증을 연결했다. 실제 지원 범위는 [구현 구조](ARCHITECTURE.md)와 [사용 안내](README.ko.md)를 기준으로 한다. 아래 기본 수치와 배정 정책은 실험으로 조정한다.
