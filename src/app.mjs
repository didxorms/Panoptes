...
import { DemoVerifier, LeanVerifier } from './lean.mjs';
...
export function createApp({
  ...
  const engine = new Engine(database, {
    ...
    liveProvider: new OpenRouterProvider({ vault }),
  });
  return { store: database, vault, engine, token, tokenFile, directory, ...verifiers };
}
